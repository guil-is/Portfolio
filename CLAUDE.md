...
- Bank data never leaves the browser (parsed client-side, kept in localStorage). Keep it that way — no API route, no upload. Permanence comes from the Back up / Restore strip on `/for/books` (`src/lib/expenses/backup.ts`, one JSON envelope); an encrypted sync, if ever built, must ship that same envelope as ciphertext only.
