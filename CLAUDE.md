...
- Pure functions in `src/lib/money/overview.ts`; balances in `src/lib/money/accounts.ts` (`books:v1:accounts`, so the sync and the backup carry them); the year maths shared with the estimate tab in `src/lib/expenses/estimate.ts`; UI under `src/components/money/`. Privacy mode (eye icon, `H`) hides amounts on that device only. The no-plaintext rule from the tax section applies: balances never go to any API in the clear.
