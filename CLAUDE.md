...
- Two layouts exist while Guil picks one: `/money` on the site's design system, `/money/v2` on **shadcn/ui** (`src/components/money/v2/FdDashboard.tsx`; components copied into `src/components/ui/`, theme tokens at the end of `globals.css`, `card`/`muted`/`accent` prefixed `fd-` to avoid collisions — see `src/components/ui/README.md`). Both read `useDashboard()`, so change numbers there, not in a layout. When he decides, delete the other layout and its route.
